...
def register():
    ...
    if request.method == 'POST':
        if register_form.validate_on_submit():
            name_var = register_form['name'].data
            email_var = register_form['email'].data
            password_var = register_form['password'].data

            new_user = User(
                name = name_var,
                email = email_var,
                password = generate_password_hash(password_var, method='pbkdf2:sha256', salt_length=8)
            )
...
def login():
    ...
    if request.method == 'POST':
        if login_form.validate_on_submit():
            email_var = login_form['email'].data
            password_var = login_form['password'].data

            result = db.session.execute(db.select(User).where(User.email==email_var))
            user = result.scalar()
            
            if not user:
                flash('Account does not exist. Try Again.')
                return redirect(url_for('login'))
            elif not check_password_hash(user.password, password_var):
